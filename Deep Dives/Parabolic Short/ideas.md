  
* if you failed to catch the short then just wait for vwap pullback (use day's high or last swing as a SL dont put SL just above vwap )
- scan for stocks that are gapping up because FRD usually always gapped up
- on FRD if gap up happens , then check for 5 min high vol in red , and may be also in green and next candle followed by red which will engulf the first green candle , if you miss the entry sometimes u can ener small and wait for vwap reclaim to add more
- Trust the math even if gets uncomfortable
- i should check dollar move from each base to next swing , so i can guess if this is the top how much sl i should keep
- when stocks gap up more than 50% and if it fails to sustain above vwap in first 20-30 mins , then its a good short trade
- in some case in parabolic move it gives short opp even before FRD , intrday short , it makes parabolic move in intraday and gives good short opp like 40% 15 min candle
- in many case FRD candle has parabolic move in intraday and that triggers the sellers so if it is becoming parabolic in intraday then just draw trendline in intraday and break of the trendline is the sign , and also volume max at the top is also a sign (use bband for extention analysis )in intraday parabolic move cover some good amt at vwap itself ( which u have sold at the top )
- 1 min chart and same applies for 5 min also

![](https://beta.appflowy.cloud/api/file_storage/476bf889-fbd8-419d-9a45-8856f645ad92/v1/blob/99a2b855-df5d-4ddc-9cb2-b1e45d223c54/hIQPVYJEWlJ7dVCV7KGD_W8HDA48MEMlw5E1C8HmdnA=.png)  

- when we use trendline break as an entry , sometimes there will be no volume on red , but but breakdown is a breakdown just enter many examples are there which had a great sell off
- trendline approach

![](https://beta.appflowy.cloud/api/file_storage/476bf889-fbd8-419d-9a45-8856f645ad92/v1/blob/99a2b855-df5d-4ddc-9cb2-b1e45d223c54/Ot_pf3Gjo87qtLtT5Hr64K8yvjkwk_R0V1ensDjjT7A=.png)

* sometimes first 3-4 candles parabolic which is a sign of reversal![[IMG-20260916201206288.png]]


* observation is during parabolic move it literally moves like a step , and major move would have happened in first early few mins itself![[IMG-20260917010225801.png]]


* even when extention is there and volume is 2x of prev candle but wick is not high then we should keep stoploss little above the high vol candle , if wick is high then we can keep high of the wick as stoploss example:
![[IMG-20260917152016471.png]]


* when we enter on the first candle high red vol our entry will be in 2nd candle so if its going up we should be careful , should be watching 1 min candle also, and WHEN I SAY FIRST HIGH RED VOL CANDLE IT HAS HIGH PROBABILTY IF WE COMBINE IT WITH ORB .

* add on vwap tap heavily , but stop should be very near to vwap ( add only after seeing red candle or higher wick in green candle near vwap , otherwise avoid it)

* Whenever you cant draw a proper trendline because of  stairs structure , just draw trendline by joining VWAP end  point of each day
![[IMG-20260918234941395.png]]

